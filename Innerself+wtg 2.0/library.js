...
globalThis.MainSettings = (class MainSettings {
    ...
    /**
     * WTG 2.0 Lightweight
     * World Time Generator - Automatic time tracking for AI Dungeon
     */
    static WTG = {
    // Is WTG enabled when the adventure begins?
    IS_WTG_ENABLED_BY_DEFAULT: true
    // (true or false)
    ,
    // Default time duration multiplier (1.0 = normal speed)
    DEFAULT_TIME_DURATION_MULTIPLIER: 1.0
    // (0.1 to 10.0)
    ,
    // Is WTG debug mode enabled by default?
    IS_WTG_DEBUG_MODE_ENABLED_BY_DEFAULT: false
    // (true or false)
    ,
    }; //——————————————————————————————————————————————————————————————————————————————
...
//—————————————————————————————————————————————————————————————————————————————————————
// WTG 2.0 Lightweight - World Time Generator Functions
//—————————————————————————————————————————————————————————————————————————————————————
...
function isLightweightMode() {
  return state.wtgMode === 'lightweight' || state.wtgMode === undefined;
}
...
function getCardTimestamp(card) {
  ...
  const match = card.entry.match(new RegExp(`(?:Discovered on|Met on|Visited) (${WTG_DATE_PATTERN})\\s+(.+)`, 'i'));
...
function ensureLeadingSpace(actionText) {
  ...
  // Check if text already starts with a space or zero-width character
  if (/^[\u200B-\u200D]/.test(actionText)) {
    return actionText;
  }
